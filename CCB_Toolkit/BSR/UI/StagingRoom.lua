...
local m_infoTabsIM = InstanceManager:new("ShellTab", "TopControl", Controls.InfoTabs);
local m_shellTabIM = InstanceManager:new("ShellTab", "TopControl", Controls.ShellTabs);
...
-- of the staging room.
local m_savePBCReadyChoice = false;    -- Should we save the user's PlayByCloud ready choice when they have decided?
local m_exitReadyWait = false;        -- Are we waiting on a local player ready change to propagate prior to exiting the match?
local m_numPlayers;
local m_teamColors = {};
local m_sessionID = FireWireTypes.FIREWIRE_INVALID_ID;
...
local m_countdownType = CountdownTypes.None;    -- Which countdown type is active?
...
local MAX_EVER_PLAYERS = 20; -- hardwired max possible players in multiplayer, determined by how many players --BUDDY: changed from 8
local MIN_EVER_PLAYERS = 2;  -- hardwired min possible players in multiplayer, the game does bad things if there aren't at least two players on different teams.
local MAX_SUPPORTED_PLAYERS = 20; -- Max number of officially supported players in multiplayer.  You can play with more than this number, but QA hasn't vetted it. -- BUDDY: changed from 8
local g_currentMaxPlayers = MAX_EVER_PLAYERS;
local g_currentMinPlayers = MIN_EVER_PLAYERS;
...
local EmptyHumanRequiredSlotStatusStr = Locale.Lookup("LOC_INVALID_SLOT_HUMAN_REQUIRED");
local EmptyHumanRequiredSlotStatusStrTT = Locale.Lookup("LOC_INVALID_SLOT_HUMAN_REQUIRED_TT");
...
local COLOR_GREEN = UI.GetColorValueFromHexLiteral(0xFF00FF00);
local COLOR_RED = UI.GetColorValueFromHexLiteral(0xFF0000FF);
local ColorString_ModGreen = "[color:ModStatusGreen]";
local PLAYER_LIST_SIZE_DEFAULT = 325;
local PLAYER_LIST_SIZE_HOTSEAT = 535;
local GRID_LINE_WIDTH = 1020;
local GRID_LINE_HEIGHT = 51;
local NUM_COLUMNS = 5;

local TEAM_ICON_SIZE = 38;
local TEAM_ICON_PREFIX = "ICON_TEAM_ICON_";


-------------------------------------------------
-- Localized Constants
-------------------------------------------------
local LOC_FRIENDS = Locale.ToUpper(Locale.Lookup("LOC_MULTIPLAYER_FRIENDS"));
local LOC_GAME_SETUP = Locale.Lookup("LOC_MULTIPLAYER_GAME_SETUP");
local LOC_GAME_SUMMARY = Locale.Lookup("LOC_MULTIPLAYER_GAME_SUMMARY");
local LOC_STAGING_ROOM = Locale.ToUpper(Locale.Lookup("LOC_MULTIPLAYER_STAGING_ROOM"));
...
function serialize (x)
    ...
    function dump_or_ref_val (x)
        if nested[x] then
            return 'false'
        end -- placeholder for recursive reference
        if not multiple[x] then
            return dump_val(x)
        end
        local var = dumped[x]
        if var then
            return "_[" .. var .. "]"
        end -- already referenced
...
-- Helper Functions
----------------------------------------------------------------
function SetCurrentMaxPlayers(newMaxPlayers)
    g_currentMaxPlayers = math.min(newMaxPlayers, MAX_EVER_PLAYERS);
end

function SetCurrentMinPlayers(newMinPlayers)
...
function IsDisplayableSlot(playerID)
...
function GetCountdownTimeRemaining()
    local countdownData = g_CountdownData[m_countdownType];
    if (countdownData == nil) then
        return 0;
    end

    if (countdownData.TimerType == TimerTypes.NetworkManager) then
        local sessionTime = Network.GetElapsedSessionTime();
...
-- Event Handlers
----------------------------------------------------------------
function OnMapMaxMajorPlayersChanged(newMaxPlayers)
...
function OnMapMinMajorPlayersChanged(newMinPlayers)
...
function OnUploadCloudPlayerConfigComplete(success)
...
function OnTeamChange(playerID, isBatchCall)
    ...
    if (pPlayerConfig ~= nil) then
        ...
        if ((g_cachedTeams[playerID] ~= nil and GameConfiguration.GetTeamPlayerCount(g_cachedTeams[playerID]) <= 0) -- was last player on old team.
                or (GameConfiguration.GetTeamPlayerCount(teamID) <= 1)) then
            -- first player on new team.
...
function OnChat(fromPlayer, toPlayer, text, eTargetType, playSounds)
    if (ContextPtr:IsHidden() == false) then
        ...
        local chatColor = "[color:ChatMessage_Global]";
        ...
        if (eTargetType == ChatTargetTypes.CHATTARGET_PLAYER) then
            local pTargetConfig = PlayerConfigurations[toPlayer];
...
function OnModStatusUpdated(playerID, modState, bytesDownloaded, bytesTotal,
                            modsRemaining, modsRequired)
	...
	if(playerEntry ~= nil) then
		if(modState ~= 1) then
			...
			if(bytesTotal > 0) then
				local bytesRemainingStr  = "";
				local modSizeStr  = BytesStr;
				local bytesDownloadedScaled  = bytesDownloaded;
				local bytesTotalScaled  = bytesTotal;
...
function OnMultiplayerHostMigrated(newHostID)
    if (ContextPtr:IsHidden() == false) then
...
-- OnJoinTeamButton
-------------------------------------------------
function OnTeamPull(playerID, teamID)
...
function OnReadyButton(playerID)
    ...
    if (not IsCloudInProgress()) then
        -- PlayByCloud match already in progress, don't touch the local ready state.
...
function OnClickToCopy()
    local sText = Controls.JoinCodeText:GetText();
...
function SetLocalReady(newReady)
    ...
    if (newReady ~= localPlayerConfig:GetReady()) then
        ...
        if (newReady
                ...
                and not m_exitReadyWait) then
            -- Do not show ready popup if we are exiting due to pressing the back button.
            ShowPBCReadyPopup();
        end
...
function ShowPBCReadyPopup()
    ...
    local readyUpBehavior = UserConfiguration.GetPlayByCloudClientReadyBehavior();
...
function CheckTeamsValid()
    m_bTeamsValid = false;
    local noTeamPlayers = false;
    local teamTest = TeamTypes.NO_TEAM;
    ...
    for i, iPlayer in ipairs(player_ids) do
        ...
        if (curPlayerConfig:IsParticipant()
            ...
            local curTeam = curPlayerConfig:GetTeam();
...
function CheckGameAutoStart()
    ...
    elseif (not Network.IsPlayerHotJoining(Network.GetLocalPlayerID())
            ...
            and not Network.IsCloudLaunching()) then
        -- We should not autostart if we are already launching into a PlayByCloud match.
        ...
        for i, iPlayer in ipairs(player_ids) do
            ...
            if ((curSlotStatus == SlotStatus.SS_TAKEN -- Human civ
                    ...
                    and (curPlayerConfig:IsAlive() or curSlotStatus == SlotStatus.SS_OBSERVER)) then
                -- Dead players do not block launch countdown.  Observers count as dead but should still block launch to be consistent.
...
function CheckLeaveGame()
    ...
    if not ContextPtr:IsHidden()    -- If the screen is not visible, this exit might be part of a general UI state change (like Multiplayer_ExitShell)
            ...
            and not Network.IsInGameStartedState() then
        -- Don't trigger leave game if we're being used as an ingame screen. Worldview is handling this instead.
...
function OnHandleExitRequest()
    ...
    for _, playerEntry in ipairs(g_PlayerEntries) do
...
function CheckShowSlotButton(slotData, playerID)
    local pPlayerConfig = PlayerConfigurations[playerID];
    local playerSlotStatus = pPlayerConfig:GetSlotStatus();
    ...
    if (slotData.slotStatus == SlotStatus.SS_CLOSED or slotData.slotStatus == SlotStatus.SS_OPEN) then
        if (playerSlotStatus == SlotStatus.SS_TAKEN or playerSlotStatus == SlotStatus.SS_COMPUTER) then
            -- Current SlotType is a civ
...
-- Team Scripting
-------------------------------------------------
function GetTeamCounts(teamCountTable)
...
function AddTeamPulldownEntry(playerID, pullDown, instanceManager, teamID, teamName)

    local instance = instanceManager:GetInstance();

    if teamID >= 0 then
        local teamIconName = TEAM_ICON_PREFIX .. tostring(teamID);
...
function SetupTeamPulldown(playerID, pullDown)
    ...
    if (not noTeams) then
        ...
        local newTeamName = tostring(newTeamID);
        newPulldownEntry = {};
    ...
    table.sort(pulldownEntries, function(a, b)
...
function UpdatePlayerEntry_SlotTypeDisabled(playerID)
    ...
    if (playerEntry ~= nil) then
        ...
        local bCanChangeSlotType = not localPlayerConfig:GetReady()
...
function UpdatePlayerEntry(playerID)
    ...
    if (playerEntry ~= nil) then
        ...
        local isHotSeat = GameConfiguration.IsHotseat();
        ...
        local isKickable = Network.IsGameHost()            -- Only the game host may kick
        ...
        local hidePlayerCard = isHotSeat or slotStatus ~= SlotStatus.SS_TAKEN;
        local showHotseatEdit = isHotSeat and slotStatus == SlotStatus.SS_TAKEN;
        ...
        if slotStatus == SlotStatus.SS_TAKEN or slotStatus == SlotStatus.SS_OBSERVER then
            local isReady = pPlayerConfig:GetReady();
        ...
        if (isActiveSlot) then
            ...
            elseif slotStatus == SlotStatus.SS_CLOSED then
                ...
                if (m_iFirstClosedSlot == -1 or m_iFirstClosedSlot == playerID)
                        ...
                        and not GameConfiguration.IsMatchMaking() then
                    -- Players can't change number of slots when matchmaking.
...
function UpdateAllDefaultPlayerNames()
    ...
    local kPlayerIDs = GameConfiguration.GetMultiplayerPlayerIDs();

    for i, iPlayer in ipairs(kPlayerIDs) do
        local pCurPlayerConfig = PlayerConfigurations[iPlayer];
        local pCurPlayerEntry = g_PlayerEntries[iPlayer];
        local slotStatus = pCurPlayerConfig:GetSlotStatus();

        -- Case where multiple times on one machine it appeared a config could exist
        -- for a taken player but no player object?
        local isSafeToReferencePlayer = true;
...
function UpdateReadyButton()
    ...
    if (m_countdownType ~= CountdownTypes.None) then
        local startLabel = Locale.ToUpper(Locale.Lookup("LOC_GAMESTART_COUNTDOWN_FORMAT"));  -- Defaults to COUNTDOWN_LAUNCH
        local toolTip = "";
        ...
        local timeRemaining = GetCountdownTimeRemaining();
        local intTime = math.floor(timeRemaining);
    ...
    elseif (not g_everyoneConnected) then
        ...
        local waitingForJoinersTooltip = Locale.Lookup("LOC_READY_BLOCKED_PLAYERS_CONNECTING_TT");
        local player_ids = GameConfiguration.GetMultiplayerPlayerIDs();
        for i, iPlayer in ipairs(player_ids) do
            local curPlayerConfig = PlayerConfigurations[iPlayer];
            local curSlotStatus = curPlayerConfig:GetSlotStatus();
    ...
    elseif (not g_everyoneModReady) then
        ...
        local waitingForModReadyTooltip = Locale.Lookup("LOC_READY_BLOCKED_PLAYERS_NOT_MOD_READY");
        local player_ids = GameConfiguration.GetMultiplayerPlayerIDs();
        for i, iPlayer in ipairs(player_ids) do
            local curPlayerConfig = PlayerConfigurations[iPlayer];
            local curSlotStatus = curPlayerConfig:GetSlotStatus();
...
-------------------------------------------------
function StartCountdown(countdownType)
...
function RealizeGridSize()
    ...
    local gridLineHeight = math.max(Controls.PlayerListStack:GetSizeY(), Controls.PlayersScrollPanel:GetSizeY());
...
-------------------------------------------------
function OnUpdateTimers(uiControl, fProgress)

    local fDTime = UIManager:GetLastTimeDelta();
...
function OnShow()
    ...
    local networkSessionID = Network.GetSessionID();
...
function OnChatPulldownChanged(newTargetType, newTargetID)
    local textControl = Controls.ChatPull:GetButton():GetTextControl();
...
function ShowHideInviteButton()
    local canInvite = CanInviteFriends(true);
...
function ShowHideTopLeftButtons()
    local showEndGame = GameConfiguration.IsPlayByCloud() and Network.IsGameHost();
    local showQuitGame = GameConfiguration.IsPlayByCloud();
...
-------------------------------------------------------------------------------
function SetupSplitLeaderPulldown(playerId, instance, pulldownControlName, civIconControlName, leaderIconControlName, tooltipControls)
    local parameters = GetPlayerParameters(playerId);
    if (parameters == nil) then
        parameters = CreatePlayerParameters(playerId);
    end
            ...
            if (v == nil) then
                ...
                if (not tooltipControls.HasLeaderPlacard) then
                    ...
                    button:RegisterCallback(Mouse.eMouseEnter, function()
                        if (info == nil) then
                            info = GetPlayerInfo(domain, value, playerId);
                        end
        ...
        UpdateValues = function(values)
            ...
            for i, v in ipairs(values) do
                ...
                entry.Button:RegisterCallback(Mouse.eLClick, function()
                    if (info == nil) then
                        info = GetPlayerInfo(domain, value);
                    end
...
function RealizeShellTabs()
    ...
    local gameSetup = m_shellTabIM:GetInstance();
    ...
    local stagingRoom = m_shellTabIM:GetInstance();
...
function ShowJoinCode()
    local pbcMode = GameConfiguration.IsPlayByCloud() and (GameConfiguration.GetGameState() == GameStateTypes.GAMESTATE_LOAD_PREGAME or GameConfiguration.GetGameState() == GameStateTypes.GAMESTATE_PREGAME);
    local crossPlayMode = (Network.GetTransportType() == TransportType.TRANSPORT_EOS);
    local eosAllowed = (Network.GetNetworkPlatform() == NetworkPlatform.NETWORK_PLATFORM_EOS) and GameConfiguration.IsInternetMultiplayer();
...
function BuildGameState()
    ...
    local joinCode = Network.GetJoinCode();
...
function UpdateFriendsList()
    ...
    for _, friend in pairs(friends) do
        local instance = m_friendsIM:GetInstance();
        ...
        -- end build
        local friendPlayingCiv = friend.PlayingCiv; -- cache value to ensure it's available in callback function

        PopulateFriendsInstance(instance, friend, friendActions,
                function(friendID, actionType)
                    if actionType == "invite" then
                        local statusText = friendPlayingCiv and "LOC_PRESENCE_INVITED_ONLINE" or "LOC_PRESENCE_INVITED_OFFLINE";
...
-------------------------------------------------
function SetupGridLines(numPlayers)
    g_GridLinesIM:ResetInstances();
    RealizeGridSize();
    local nextY = GRID_LINE_HEIGHT;
    local gridSize = Controls.GridContainer:GetSizeY();
    local numLines = math.max(numPlayers, gridSize / GRID_LINE_HEIGHT);
    for i = 1, numLines do
...
-- 申请玩家标记页面
-- =====================================================================
function OnOutputButtonPlayerRequest()
    -- 申请玩家标记
    UI.PlaySound("Play_UI_Click");
...
function OnCheckPlayerData(playerID)
    if playerID then
        ...
        if playerEntry.StatusLabel:GetText() ~= NotReadyStatusStr and playerEntry.StatusLabel:GetText() ~= ReadyStatusStr and playerEntry.StatusLabel:GetText() ~= Locale.Lookup("LOC_MP_PLAYER_CONNECTED_SUMMARY") then
            -- 只有在就绪和未就绪时，替换文本
            return
        end
        local pPlayerConfig = PlayerConfigurations[playerID];
        if (pPlayerConfig:IsHuman()) then
            ...
            if (string.len(playerNetworkID) ~= 17 and string.len(playerNetworkID) ~= 32) then
                -- 无法获取ID时
            ...
            for i, kData in ipairs(g_TPT_PlayerDatas) do
                if playerNetworkID == kData.SteamID then
                    -- SteamID匹配
    ...
    for _, curMod in ipairs(InstalledMods) do
        if not curMod.Official then
            ...
            for _, InUseMod in ipairs(EnabledMods) do
                if curMod.Id == InUseMod.Id then
                    -- 房间中在使用的
                    local IsSubscribed = false
                    for _, item in ipairs(SubscribedMods) do
                        if curMod.SubscriptionId == item then
                            -- 排除已经订阅的物品
                            IsSubscribed = true
                        end
                    end
                ...
                if curMod.Id == InUseMod.Id then
                    ...
                    if not IsSubscribed then
                        ...
                        if curMod.SubscriptionId and curMod.SubscriptionId ~= "" then
                            if tonumber(v.SubscriptionId) > 3240000000 then
                                -- 比较新的模组
...
function OnChangeName_ConfirmButton()
    ...
    LuaEvents.JoiningRoom_ShowStagingRoom.Add(function()
        str = nil
    end)
    if str ~= nil then
...
function Initialize()
    ...
    Controls.InviteButton:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over");
    end);
    Controls.EndGameButton:RegisterCallback(Mouse.eLClick, OnEndGameAskAreYouSure);
    Controls.EndGameButton:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over");
    end);
    Controls.QuitGameButton:RegisterCallback(Mouse.eLClick, OnQuitGameAskAreYouSure);
    Controls.QuitGameButton:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over");
    end);
    Controls.ReadyButton:RegisterCallback(Mouse.eLClick, OnReadyButton);
    Controls.ReadyButton:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over");
    end);
    Controls.ReadyCheck:RegisterCallback(Mouse.eLClick, OnReadyButton);
    Controls.ReadyCheck:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over");
    end);
    ...
    Controls.OutputButton:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over");
    end);
    Controls.OutputButton:RegisterCallback(Mouse.eMouseExit, function()
        Controls.PlayerRequestTip:SetHide(true);
    end);
    ...
    Controls.CloseAIButton:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over");
    end);

    Controls.TeamPVPHelpButton:RegisterCallback(Mouse.eLClick, OnTeamPVPHelpButton);
    Controls.TeamPVPHelpButton:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over");
    end);
    ...
    Controls.RandomTeamButton:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over");
    end);
    Controls.RandomTeamButton:RegisterCallback(Mouse.eMouseExit, function()
        Controls.RandomTeamTip:SetHide(true);
    end);

    Controls.SettingNameButton:RegisterCallback(Mouse.eLClick, OnSettingNameButton);
    Controls.SettingNameButton:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over");
    end);
    ...
    Controls.ModCheckButton:RegisterCallback(Mouse.eMouseExit, function()
        Controls.ModCheckTip:SetHide(true);
    end);
    ...
    Controls.FreeChooseAbilityCivilizationButton:RegisterCallback(Mouse.eMouseEnter, function()
        UI.PlaySound("Main_Menu_Mouse_Over");
    end);

    Controls.LeaderStatsButton:SetHide(true)
    ...
    Controls.SteamIDInputEditBox:RegisterStringChangedCallback(function()
        ...
        if str and #str == 17 then
            ...
            local steamIds = GetLocalSteamIDsData()
...
local m_HandCardsIM = InstanceManager:new("HandCardInstance", "Content", Controls.CardStack);        -- 手牌
local m_DeclarerHandCardsIM = InstanceManager:new("HandCardInstance", "Content", Controls.DeclarerCardStack);        -- 庄家手牌
...
local IMG_POLICYCARD_BY_ROWIDX = {};                        -- 卡牌花色贴图
...
function Restart()
    ...
    elseif os.time() > Date + 86400 then
        -- 24小时
...
function DeclarerStart()
    ...
    if #m_Declarer_Hand_PokeCard_Data == 1 then
        -- 第一次抽牌
...
function RealizeDeclarerHandCards(Data, CanShow)
    ...
    for i, v in pairs(Data) do
        ...
        if not (i == 1 and #Data == 2 and not CanShow) then
            -- 不是第一张牌才显示花色和点数
...
function DeclarerLogicLoop()
    ...
    if DeclarerBestNum >= 17 then
        if DeclarerBestNum < PlayerBestNum and DeclarerMin < 17 then
            -- 如果庄家现有点数小于玩家，但是有A,最小点数小于17，庄家再继续抽牌
            DeclarerStart()
...
-- 结算环节
-- ====================================================
function DeclarerStop(DeclarerBestNum, PlayerBestNum)
    -- 庄家停牌了
    m_Win = false;
    if m_InProgress then
        if DeclarerBestNum > 21 then
            ...
            if PlayerBestNum > DeclarerBestNum then
                -- 玩家点数大于庄家
...
function Poker_OnPlayerInfoChanged_Refresh(playerID)
    ...
    if tValues[1] ~= GOLD_Balanced_Cache[playerID] then
        -- 校验
...
-- 永久储存玩家筹码信息
-- =======================================================================
function SetstorageGold(newdate)
    -- 如果是签到奖励，则要更新日期，否则用旧日期
